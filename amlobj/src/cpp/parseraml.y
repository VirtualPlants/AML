...
static void initLogFile() {
  ...
  if (log_file) {
    ...
    *log_file << "#----------------------------------------------------------------#" << std::endl;
    *log_file << "# LOG FILE : " << __DATE__ << " created by " << TOOLS(getUserName()).c_str() << std::endl;
    *log_file << "# File Generated by AMAPmod " << release_number() << " on "<< TOOLS(getOSName()).c_str() << std::endl;
    *log_file << "#----------------------------------------------------------------#" << std::endl;
...
bool push_File(const char* file,bool inc = false){
  ...
  string new_cwd = TOOLS(get_dirname)(file);     // gets the new working dir name
  string _cwd = get_cwd();
